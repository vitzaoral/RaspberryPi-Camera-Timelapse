...
3. GET beeSys outdoor temperature (3 attempts); its HTTP `Date` header is the clock reference — if the system clock is off by > 60 s, set it and write the WittyPi RTC (`clock.py`; fixes frozen RTC after a power cut on a dead supercap, no manual force-sync needed). If that GET failed, the telemetry POST at the end of the cycle is the second chance: the clock is fixed then and the note is carried to the next cycle's log (`clock_note.txt`)
